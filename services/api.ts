...
const normalizeNotificationPreferences = (preferences: any): NotificationPreferences => Object.keys(DEFAULT_NOTIFICATION_PREFERENCES).reduce((result, key) => {
...
const normalizeRoom = (room: any): Room => {
    ...
    return {
        ...
        availableSpaces: room?.availableSpaces ?? Math.max(0, (room?.capacity ?? 0) - currentOccupancy),
...
const normalizeInvitationHistoryEntry = (entry: any): InvitationHistoryEntry => ({
    ...
    bunkNumber: entry?.bunkNumber != null
...
const normalizeNotificationSettings = (settings: any): NotificationSettings => {
    ...
    return {
        ...
        hasActiveDevice: Boolean(settings?.hasActiveDevice ?? devices.some((device: {
            enabled: boolean;
        }) => device.enabled)),
...
const normalizeDashboard = (dashboard: any): DashboardData => ({
    ...
    hasReservation: dashboard?.hasReservation ??
...
const normalizePaymentStatus = (status: any): PaymentStatus => {
    ...
    const mappedStatus = normalizedStatus === 'completed' || normalizedStatus === 'success'
        ? 'paid'
        : normalizedStatus;
    return {
        ...status,
...
export const studentAPI = {
    ...
    getHostels: () => api.get<ApiResponse<Hostel[]>>('/student/hostels'),
    ...
    reserveRoom: (data: ReservationRequest) => api.post<ApiResponse<Reservation>>('/student/reservations', {
        roomId: data.roomId,
    ...
    getAlerts: () => api.get<ApiResponse<Alert[]>>('/student/alerts'),
    ...
    updateNotificationPreferences: async (preferences: Partial<NotificationPreferences>) => {
        const response = await api.patch<ApiResponse<NotificationSettings>>('/student/notifications/preferences', preferences);
        const settings = normalizeNotificationSettings(unwrapData<any>(response.data));
        return {
            ...response,
            data: {
    ...
    unregisterPushDevice: (token: string) => api.delete<ApiResponse<NotificationSettings>>('/student/notifications/devices', {
        data: { token },
    }),
    ...
    uploadProfilePicture: async (formData: FormData) => {
        const token = await AsyncStorage.getItem('token');
        const response = await fetch(`${API_CONFIG.BASE_URL}/student/profile/picture`, {
            method: 'POST',
            headers: {
