...
export const isPushNotificationsSupported = () => isNativePushPlatform && !isExpoGo;
...
export const registerForPushNotificationsAsync = async (preferences?: Partial<NotificationPreferences>): Promise<PushRegistrationResult> => {
    const Notifications = getNotificationsModule();
    if (!Notifications) {
        return {
            status: 'unsupported',
            message: getPushNotificationsUnavailableReason() ??
...
export const unregisterStoredPushTokenAsync = async (unregisterer?: (token: string) => Promise<unknown>) => {
