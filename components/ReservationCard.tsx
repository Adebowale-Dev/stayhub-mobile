...
export function ReservationCard({ reservation, onViewDetails, onCancel }: Props) {
    return (<Card style={styles.card}>
      <Card.Content>
        ...
        {reservation.groupMembers && reservation.groupMembers.length > 0 && (<View style={styles.row}>
      ...
      {(onViewDetails || onCancel) && (<Card.Actions>
          {onViewDetails && (<Button onPress={onViewDetails} compact>Details</Button>)}
          {onCancel && reservation.status !== 'cancelled' && (<Button onPress={onCancel} compact textColor="#e53935">Cancel</Button>)}
        </Card.Actions>)}
    </Card>);
}
