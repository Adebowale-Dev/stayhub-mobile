...
export function RoomCard({ room, onReserve }: Props) {
  ...
  return (
    <Card style={[styles.card, !available && styles.dimmed]}>
      <Card.Content>
        <View style={styles.header}>
          ...
          <Badge
            label={available ? 'Available' : room.status === 'maintenance' ? 'Maintenance' : 'Full'}
            variant={available ? 'success' : room.status === 'maintenance' ? 'warning' : 'error'}
          />
        </View>
