...
import { View, StyleSheet, Alert, Keyboard, KeyboardAvoidingView, TouchableWithoutFeedback, Platform, ScrollView, TouchableOpacity, StatusBar, ImageBackground, Dimensions, } from 'react-native';
    ...
    return (<TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        <ImageBackground source={BG_IMAGE} style={styles.bg} resizeMode="cover">
          ...
          <ScrollView style={styles.scroll} contentContainerStyle={[styles.scrollContent, { paddingTop: insets.top + 24 }]} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag" showsVerticalScrollIndicator={false} bounces={false}>
            ...
            <View style={[styles.card, { paddingBottom: insets.bottom + 28 }]}>
              ...
              <TextInput mode="outlined" label="Matric Number" value={matricNumber} onChangeText={setMatricNumber} autoCapitalize="characters" autoCorrect={false} disabled={loading} placeholder="e.g. CSC/2020/001" style={styles.input} outlineColor="#E0E0E0" activeOutlineColor="#1565C0" left={<TextInput.Icon icon="card-account-details-outline"/>}/>

              <TextInput mode="outlined" label="Password" value={password} onChangeText={setPassword} secureTextEntry={!showPassword} disabled={loading} style={styles.input} outlineColor="#E0E0E0" activeOutlineColor="#1565C0" left={<TextInput.Icon icon="lock-outline"/>} right={<TextInput.Icon icon={showPassword ? 'eye-off-outline' : 'eye-outline'} onPress={() => setShowPassword((v) => !v)}/>}/>

              <TouchableOpacity style={styles.forgotWrap} onPress={() => router.push('/(auth)/forgot-password')} disabled={loading} activeOpacity={0.7}>
