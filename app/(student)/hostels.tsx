...
function SkeletonCard({ surface }: { surface: string }) {
  return (
    <View style={[skeletonStyles.card, { backgroundColor: surface }]}>
...
export default function HostelsScreen() {
  ...
  // Apply gender lock + search filter
  ...
  // Summary stats for hero (scoped to student's gender)
  const totalAvailable = filtered.reduce((sum, h) => sum + h.availableRooms, 0);

  // ── List header (hero + filters + search) ──
  const ListHeader = () => (
    <>
      {/* Hero */}
        ...
        <View style={styles.errorWrap}>
          <View style={[styles.errorCard, { backgroundColor: theme.colors.surface }]}>
            ...
            <TouchableOpacity
              ...
              onPress={() => { setLoading(true); loadHostels(); }}
              activeOpacity={0.82}
            >
    ...
    return (
      <View style={[styles.screen, { backgroundColor: theme.colors.background }]}>
        ...
        <View style={{ paddingHorizontal: 18, paddingTop: 18, gap: 14 }}>
          {[1, 2, 3].map((i) => (
            <SkeletonCard key={i} surface={theme.colors.surface} />
          ))}
        </View>
      </View>
    );
