...
function WhatsAppTabBar({ state, descriptors, navigation }: BottomTabBarProps) {
    ...
    return (<View style={[
      ...
      {state.routes.map((route) => {
            ...
            return (<Pressable key={route.key} onPress={onPress} onLongPress={() => navigation.emit({ type: 'tabLongPress', target: route.key })} style={styles.tabItem} accessibilityRole="button" accessibilityState={isFocused ? { selected: true } : {}} accessibilityLabel={options.tabBarAccessibilityLabel} android_ripple={{ color: isDark ? 'rgba(66,165,245,0.1)' : 'rgba(21,101,192,0.08)', borderless: true, radius: 32 }}>
            <MaterialCommunityIcons name={isFocused ? tab.focused : tab.unfocused} size={28} color={isFocused ? activeColor : INACTIVE_COLOR}/>
...
export default function StudentLayout() {
    ...
    return (<Tabs tabBar={(props) => <WhatsAppTabBar {...props}/>} screenOptions={{
